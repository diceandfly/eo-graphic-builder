...
## 166. 2026-08-26 — 도움말에 UI 아이콘 인라인 (§157 확장)

- MD에 `{icon:이름}` 토큰 → 렌더 시 icons.js 사전의 실제 UI 아이콘 SVG로 치환 (문서와 UI가 같은 아이콘 원본 공유 — 아이콘 바뀌면 도움말도 자동 갱신). 오타 토큰은 그대로 노출되어 즉시 발견.
- 배치 37곳: 단축키 표(도구·삭제·뒤집기·내보내기), 파일 바 4종, 우클릭 메뉴, 도구 바 표 5종, comp dir/sym 화살표, 링크 사슬, 정렬/등간격 8종, 보기 옵션 3종, 저장/열기.
- 행간 보존 설계: 13px 인라인 + vertical-align -2px — 라인박스(12px·1.7) 내에 수용. 검증: 단축키 표 행 높이 32px 균일(아이콘 유무 무관), 잔존 토큰 0.

## 167. 2026-08-26 — 도움말 3열 표 컬럼 배분

- 표별 클래스(t-layout/t-tools/t-view, 첫 헤더로 식별) + table-layout fixed:
  - 화면 구성: 위치 84(+2글자)·이름 132 — 둘 다 무줄바꿈, 역할이 나머지 흡수
  - 도구 바: 도구 128 무줄바꿈·우클릭 260(+2글자), 좌클릭이 흡수
  - 보기 옵션: 버튼 158 무줄바꿈·좌클릭 268(+4글자), 우클릭이 흡수
- 검증: 세 표 지정 폭 적용·1열(및 이름 열) 오버플로 0, 스크린샷 확인.

## 168. 2026-08-26 — 도움말 상호 참조 링크 보강

- 타 항목 언급 키워드 전수 점검 후 17곳 추가 링크: 시작하기 개념 4종(형태/프레임/배치·정렬/내보내기), 화면 구성 표의 목적지 5셀, 코너 바 언급 2곳, 그룹 언급 3곳, 프레임 내보내기·테두리 색, 정렬·복사·스레드·우클릭 메뉴 참조. 총 76링크, 앵커 전수 매칭.

## 169. 2026-08-26 — 도움말 아이콘 2차 필터링 (37→56)

- 누락 보강 19곳: Register unit preset(presetAdd)·복사/PNG/내보내기 분해 표기(duplicate/imagePng/exportSvg — §1 표·§5 우클릭 메뉴·§15)·상하 뒤집기(flipV 추가)·선택 박스 버튼 3종·Alt드래그 복제·시작하기 저장·프레임 버튼/선택 도구·리소스 모니터의 도움말 버튼·초기화(resetArrow+tombstone).
- 검증: 아이콘 56개 렌더·잔존 토큰 0·링크 앵커 무결.

## 170. 2026-08-26 — 도움말 아이콘 3차: 소제목·불렛 시작 위치 (56→68)

- 규칙 확정(사용자): 본문 중 반복 등장엔 안 붙이고, **소제목(h3)·불렛 시작 위치**에는 매칭 아이콘 부착. h2는 목차·상호 링크 앵커가 걸려 있어 제외(슬러그 보호).
- 추가 11곳: §5 선택·복사/붙여넣기, §8 스포이드·블렌드·그리드 배열, §9 프레임 선택 모드, §10 링크, §11 프레임 기준 정렬, §15 저장·열기/내보내기/초기화.

## 171. 2026-08-26 — 도움말 아이콘 4차: 마우스 글리프·소제목 보강·§15 정리 (68→85)

- **신규 글리프 5종**(도움말 전용, icons.js §171 표기): mouseL/mouseR(좌/우클릭 — 버튼 셀 강조 마우스)·move(십자 화살표)·resize(양방향 대각)·group(겹친 사각 2).
- 좌클릭(3)/우클릭(14) 표기 전반에 마우스 아이콘 프리픽스(표 헤더 포함). 소제목 추가: §5 이동·크기/회전, §10 그룹, §14 등록(시작 위치로 이동)·삭제. **§15 소제목 아이콘은 전부 삭제**(사용자 지시).
- 총 85곳, 잔존 토큰 0.

## 172. 2026-08-26 — 마우스 글리프 개선 (§171 개정)

- 형태: 각진 바디+미세 바 2개 → **캡슐 바디 + 클릭 셀 대각 마크** (판독성 개선).
- 여백: 24 뷰박스 좌우 패딩이 옆 텍스트와의 과대 간격 원인 — 도움말 렌더에서 **뷰박스 크롭(5 1 14 22) + 비율 폭(8px)** 적용(ICON_CROP 맵, 다른 아이콘 불변).

## 173. 2026-08-26 — 도움말 3열 표 배분 미세 조정 (§167 개정)

- 도구 바: 좌클릭 열 확대(→250)·우클릭 축소(260→215). 보기 옵션: 좌클릭 축소(268→240)·우클릭 축소(→155), 버튼 열이 잔여 흡수(158→198, 무줄바꿈 유지).

## 174. 2026-08-26 — 마우스 글리프 여백 미세 조정

- 좌측 여백 0→3px(앞 텍스트와 분리), 우측 3→1px(뒤 텍스트 밀착). 크롭 글리프(mouseL/R) 한정, 일반 아이콘 불변.

## 175. 2026-08-26 — 도구/보기 표 배분 재조정 (§173 개정)

- 도구 바: 좌클릭 250→215(잔여)·우클릭 215→250 — 스포이드 범주 나열이 2줄로. 보기 옵션: 좌클릭 240→170(최장 셀 2줄)·우클릭 155→230(최장 나열 3줄), 버튼 열 잔여 193.

## 176. 2026-08-26 — 마우스 클릭 셀 채움·도구 표 배분 정정

1. **마우스 글리프** — 사선 마크 → **클릭 버튼 셀 솔리드 채움**(ICON_FILL 경로 + .fillP fill:currentColor — 전역 stroke 사전과 충돌 없이 도움말 렌더에서만). 그림체(캡슐 바디) 유지.
2. **도구 바 표 정정(사용자 정정)** — §175에서 반대로 갔던 배분을 좌클릭 270·우클릭 195로 (Frame 설명 2줄대).

## 177. 2026-08-26 — 도구/보기 표 1~2글자 미세 조정

- 도구 바: 좌클릭 270→258(-1글자)·우클릭 195→207(+1글자). 보기 옵션: 좌클릭 170→194(+2글자)·우클릭 230→206(-2글자).

## 178. 2026-08-26 — 아이콘-텍스트 간격 축소

- 도움말 인라인 아이콘 우측 여백 3→2px (마우스 글리프는 §174의 1px 유지).

## 179. 2026-08-26 — 링크 앞 아이콘 링크 컬러 연동

- 바로 뒤 요소가 링크(a)인 아이콘은 `:has(+ a)`로 액센트 컬러 적용 — 아이콘+링크가 한 묶음으로 보임 (18곳 자동 적용, 이후 추가분도 자동).

## 180. 2026-08-26 — 화면 구성 파일 바 행 4버튼 링크

- 도움말→§13(우클릭 기능 설명 위치) · 저장/열기→§15 소제목(h3 앵커) · 초기화→§15 초기화 소제목. h3 앵커 직결 첫 사례 — 아이콘은 §179 규칙으로 자동 액센트.

## 181. 2026-08-26 — 마우스 글리프 내부 우측 여백·매뉴얼 작성 가이드 내장

1. **마우스 글리프** — 크롭 뷰박스 14→16u 폭: 우측 2u(≈1px)를 글리프 내부 여백으로 복원 (§178 전역 1px 축소와 균형).
2. **매뉴얼 작성 가이드** — MANUAL.md 최상단에 **HTML 주석 블록**(도움말 창 미렌더)으로 내장: 로드 구조(?raw·HMR·자동배포)·섹션/앵커 규칙(h2 슬러그 주의·h3 자유)·문체(개발 용어 치환표·활성 조건 금지)·아이콘 토큰 규칙·링크 슬러그 규칙·표 형식(2열 자동/3열 클래스)·갱신 후 체크리스트. 애니메이션 등 후속 기능 매뉴얼 갱신 시 준거.
- 검증: 도움말 창에 가이드 미노출·본문 첫 줄 정상, 마우스 폭 9px(내부 여백 포함).
...
## 185. 2026-08-26 — 단축키 모음 표 아이콘 제거

- §1 단축키 모음의 동작 설명 앞 아이콘 11개 제거(표가 이미 키 중심이라 불필요) — 본문 섹션들의 아이콘은 유지. 아이콘-텍스트 공백은 §184에서 전수 제거 확인(잔여 간격 = CSS 1px 마진, 문자 아님).

## 186. 2026-08-26 — 단축키 표 아이콘 복원·§15 단축키 앞 아이콘 제거·간격 3px 복귀

- §185의 오해 정정: **단축키 모음 표 아이콘 복원**(11개), 대신 **§15 텍스트 설명의 단축키 표기(⇧E·⌘C·⌘⇧C) 앞 아이콘 3개 제거**.
- 아이콘-텍스트 간격: 소스 공백 전무 상태에서 CSS 마진 1→**3px**(마우스 글리프는 인라인 2px + 내부 여백 1px = 총 ~3px 동등).

## 187. 2026-08-26 — 아이콘-텍스트 간격 최종 4px

- 3→4px 확정 (마우스 글리프 인라인 3px + 내부 여백 1px = 동등).

## 188. 2026-08-26 — 마우스 글리프 1px 좌측 시프트

- 인라인 마진 좌 4→3·우 3→4 — 점유 폭 불변, 글리프만 1px 좌로.

## 189. 2026-08-26 — 마우스 글리프 좌측 시프트 2차 (누적 2px)

- 인라인 마진 좌 2·우 5px (점유 폭 불변).

## 190. 2026-08-26 — 마우스 클릭 셀 채움 1u 인셋 (착시 보정)

- 셀 경계(중앙선·디바이더·외곽)까지 꽉 채우던 솔리드가 스트로크와 합쳐져 커 보이는 착시 — 채움을 1u 인셋한 축소 쿼터로 교체.

## 191. 2026-08-26 — 링크 앞 연속 아이콘 전체 하이라이트

- `:has(+ .mdIco + a)` 추가 — 좌우/상하 뒤집기처럼 아이콘 2개가 링크 앞에 연속될 때 둘 다 액센트.

## 192. 2026-08-26 — 마우스 클릭 채움 축소 2차

- 채움을 셀 인셋 쿼터(§190)에서 **셀 안쪽 소형 마크**(약 3×4.5u 파이)로 추가 축소 — 칠한 버튼이 과대해 보이는 착시 해소.

## 193. 2026-08-26 — 마우스 착시 보정 방식 변경: 분할선 이동 (§190·§192 대체)

- 채움 축소(스팟화 부작용) 철회 → **분할선을 칠한 쪽으로 1u 이동**(mouseL x11·mouseR x13) + 해당 축소 셀 전체 채움. 칠한 버튼이 기하학적으로 작아져 시각 무게 균형.

## 194. 2026-08-26 — 마우스 채움 오버랩 확대·아이콘 간격 3px 확정

1. **채움 확대** — §193 셀 채움이 스트로크와 살짝 떠 보이던 이음새 빈틈 제거: 셀 경계보다 0.5~1u 크게 그려 스트로크 밑으로 오버랩 (스트로크가 위에 그려져 경계는 선명).
2. **간격** — 아이콘-텍스트 4→**3px 확정** (마우스는 시프트 관계 유지: ml 1·mr 4).

## 195. 2026-08-26 — 마우스 글리프 좌측 잘림 픽스

- 원인: 크롭 뷰박스(x5~)가 바디 패스(x5)의 스트로크 바깥 절반(±1u)을 왼쪽에서 잘랐음 — 마우스 전용 크롭이 스트로크 두께를 미고려. 크롭을 (3.5, 0.5, 18, 23)으로 확장: 스트로크 전체 포함 + 상하좌 0.5u·우 1.5u 패딩. 일반 아이콘은 24 정사각 뷰박스로 규격 상이(§172 의도) — 이제 잘림 없이 동작.

## 196. 2026-08-26 — 마우스 분할선 1u 추가 이동 (누적 2u)

- mouseL 분할선 x11→10 · mouseR x13→14, 채움도 새 경계에 맞춰 갱신(오버랩 유지).
